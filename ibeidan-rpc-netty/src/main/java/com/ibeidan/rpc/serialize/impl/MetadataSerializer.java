...
import com.ibeidan.rpc.nameservice.Metadata;
...
public class MetadataSerializer implements Serializer<Metadata> {
    ...
    public Metadata deserialize(byte[] bytes, int offset, int length) {
        ...
        for (int i = 0; i < sizeOfMap; i++) {
            ...
            byte [] keyeBytes = new byte[keyLength];
            buffer.get(keyeBytes);
            String key = new String(keyeBytes,StandardCharsets.UTF_8);
